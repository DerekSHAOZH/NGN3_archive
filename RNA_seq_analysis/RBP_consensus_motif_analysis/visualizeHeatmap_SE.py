...
dire="/project/Neurodifferentiation_System/Analysis_NGN3/IsoSwitches_RBPs/rMAPS/Results/"
...
for direction in m.keys():
    for i in range(len(days)-1):
        ...
        for j in range(i+1, len(days)):
            ...
            input_fn = dire+"%s_background_%s/SE/pVal.%s.vs.bg.RNAmap.txt"%(condition,control,direction)
            df = pd.read_csv(input_fn, sep = "\t")
            df=df[(df.filter(like='smallest_p_in_').min(1)<threshold)]
            df['RBP']=df["RBP"].str.split('.',expand=True)[0]
            df=df.sort_values("RBP").groupby('RBP').min()
            vals=-1*np.log10(df)
            for col in df.columns:
                df[col]=df.apply(lambda x: x.name if x[col]<threshold else '',1)
            df.columns=df.columns.str.replace('smallest_p_in_','')

            fig = plt.figure(figsize=(8, df.shape[0]*0.3)) 
            plt.suptitle("RBP motif enriched for exons %s at %s vs %s" %(m[direction],condition,control),fontsize=16)
            ...
            plt.savefig(dire+"Plots/SE_%s_%s_vs_%s.pdf" %(m[direction],condition,control))